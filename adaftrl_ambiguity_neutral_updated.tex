...
\subsection{The specific unbiased surrogate used in the statistical core}

We now specialize the general identity to the lower-bound miscoverage event. The quantity defined in this subsection is the unbiased IPCW surrogate that drives the shared statistical core of the manuscript. In the explicit AdaFTRL section we will also introduce a second, ambiguity-neutral feedback signal for the online update itself, but the latent-to-observable bridge and the martingale transfer remain anchored to the unbiased IPCW quantity defined here.
...
\paragraph{Intuition.} Corollary~\ref{cor:unbiased} tells us that the observable quantity $\hat e_t$ is the correct replacement for the hidden indicator $\mathbf 1\{T_t< L_t\}$ at the level of conditional means. We cannot observe the hidden indicator directly, but we can observe $\hat e_t$, and its conditional expectation is exactly the same. In later sections, all latent-coverage guarantees are transferred through this IPCW quantity. The AdaFTRL update below will deliberately use a more conservative feedback signal on ambiguously censored rounds, but the proof of latent coverage still returns to the unbiased surrogate $\hat e_t$ defined here.
...
The shared statistical core continues to use the unbiased IPCW surrogate
\[
\hat e_t=\frac{\Delta_t}{G_t(Y_t)}\,\1\{Y_t<L_t\},
\qquad
\bar g_t=g_{\min}(\hat e_t-\alpha).
\]
This is the right sign for the statistical core because positive \(\bar g_t\) means the observable IPCW error is above target.

For the AdaFTRL update itself, however, it is useful to distinguish two different kinds of censored rounds below the played lower bound. If \(Y_t\ge L_t\), then the round is definitively favorable: because \(T_t\ge Y_t\), we already know that \(T_t\ge L_t\). By contrast, if \(\Delta_t=0\) and \(Y_t<L_t\), then the round is ambiguous: censoring happened below the bound, so the latent event \(\{T_t<L_t\}\) is neither confirmed nor ruled out. The raw IPCW surrogate assigns the value \(0\) in that ambiguous case, which makes a direct online update drift upward under heavy censoring.

To remove that drift, the AdaFTRL dynamics use the ambiguity indicator
\[
A_t:=\1\{\Delta_t=0,\ Y_t<L_t\}
\]
and the ambiguity-neutral feedback
\[
\hat e_t^{\mathrm{ad}}
:=
\hat e_t+\alpha A_t
...
\begin{cases}
0, & Y_t\ge L_t,\\[1ex]
\dfrac{1}{G_t(Y_t)}, & \Delta_t=1,\ Y_t<L_t,\\[2ex]
\alpha, & \Delta_t=0,\ Y_t<L_t.
\end{cases}
\]
Accordingly, define the AdaFTRL update gradient by
\[
\bar g_t^{\mathrm{ad}}:=g_{\min}(\hat e_t^{\mathrm{ad}}-\alpha),
\qquad
h_t:=-\bar g_t^{\mathrm{ad}}=g_{\min}(\alpha-\hat e_t^{\mathrm{ad}}).
\]
This gives the desired three-way behavior:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item if \(Y_t\ge L_t\), then \(h_t=\alpha g_{\min}>0\), so the method moves toward a larger lower bound;
  \item if \(\Delta_t=1\) and \(Y_t<L_t\), then \(h_t=g_{\min}(\alpha-1/G_t(Y_t))\), so an observed failure pushes in the conservative direction;
  \item if \(\Delta_t=0\) and \(Y_t<L_t\), then \(h_t=0\), so an ambiguously censored round produces no drift at all.
\end{itemize}
In particular, under complete censoring below the currently played lower bound, the AdaFTRL state remains fixed instead of being pushed away from its target-aligned anchor.
...
If recent observed failures have been too frequent, then the corrected excesses \(\hat e_t^{\mathrm{ad}}-\alpha\) have often been positive, so the internal gradients \(h_t=g_{\min}(\alpha-\hat e_t^{\mathrm{ad}})\) have often been negative. That makes \(H_{t-1}\) negative, which makes \(a_t\) larger. This decreases \(\tau_t\) and therefore decreases the lower predictive bound, which is the correct response to under-coverage. By contrast, ambiguously censored rounds with \(\Delta_t=0\) and \(Y_t<L_t\) contribute exactly zero gradient, so they do not create artificial upward drift. On the first round, \(H_0=0\) and \(\Lambda_1=1\), so \(a_1=a_{\mathrm{init}}\) and therefore \(\tau_1=\alpha\).
...
\For{$t=1,2,\dots$}
  ...
  \State Compute the unbiased IPCW surrogate
  \[
  \hat e_t=\frac{\Delta_t}{G_t(Y_t)}\,\1\{Y_t< L_t\}.
  \]
  \State Compute the ambiguity indicator
  \[
  A_t=\1\{\Delta_t=0,\ Y_t<L_t\}.
  \]
  \State Compute the AdaFTRL feedback
  \[
  \hat e_t^{\mathrm{ad}}=\hat e_t+\alpha A_t.
  \]
  \State Compute the AdaFTRL update excess
  \[
  \bar g_t^{\mathrm{ad}}=g_{\min}(\hat e_t^{\mathrm{ad}}-\alpha).
  \]
...
The maintained dynamic state is still only the pair $(H_{t-1},S_{t-1})$. The prediction step combines that state with the fixed anchor \(a_{\mathrm{init}}\) to choose \(a_t\), then maps \(a_t\) to the played quantile level \(\tau_t\), and finally queries the predictive model at that level. The update step first forms the unbiased IPCW surrogate \(\hat e_t\), then replaces the ambiguous censored-below-the-bound case by the neutral value \(\alpha\) to obtain \(\hat e_t^{\mathrm{ad}}\), and finally converts that corrected feedback into the internal gradient used by AdaFTRL.
...
\Lambda_t:=\sqrt{1+\sum_{s=1}^{t-1}h_s^2},
\]
with internal gradients \(h_t=-\bar g_t^{\mathrm{ad}}\). Then for every comparator \(a\ge 0\),
\[
\sum_{t=1}^T \bar g_t^{\mathrm{ad}}(a-a_t)
\le
\frac{\Lambda_{T+1}}{2}(a-a_{\mathrm{init}})^2+\frac12\sum_{t=1}^T\frac{(\bar g_t^{\mathrm{ad}})^2}{\Lambda_t}.
\]
\end{theorem}
...
\]
Since \( |\bar g_t^{\mathrm{ad}}|\le 1 \), this implies the fully explicit comparator envelope
\[
\sum_{t=1}^T \bar g_t^{\mathrm{ad}}(a-a_t)
\le
...
\begin{theorem}[Explicit AdaFTRL surrogate control with ambiguity-neutral feedback]
\label{thm:adaftrl-surrogate-main}
Under Assumptions~\ref{ass:monotone}, \ref{ass:gmin}, and \ref{ass:score-positive}, define
\[
A_t:=\1\{\Delta_t=0,\ Y_t<L_t\},
\qquad
A_T:=\sum_{t=1}^T A_t,
...
and
\[
\bar S_T^{\mathrm{ad}}:=\sum_{t=1}^T\bar g_t^{\mathrm{ad}},
\qquad
B_T^{\mathrm{ad}}:=\sum_{t=1}^T \hat e_t^{\mathrm{ad}} b_t
=\sum_{t=1}^T \bigl(\widetilde W_t+\alpha A_t\bigr)b_t.
\]
...
+
2\lambda_T\Bigl(g_{\min}B_T^{\mathrm{ad}}+\frac{1+\sqrt{2}}{2}(\lambda_T-1)\Bigr)
}.
\]
Consequently,
\[
...
\paragraph{Proof sketch.}
Apply Corollary~\ref{cor:adaftrl-envelope-main} inside the corrected black-box Fenchel reduction proved in Appendix~\ref{app:adaftrl-details}. On the positive branch the shifted conjugate is affine-plus-quadratic,
\[
z\mapsto a_{\mathrm{init}}z+\frac{z^2}{2\lambda_T}-\frac{1+\sqrt{2}}{2}(\lambda_T-1),
\]
and solving the resulting quadratic inequality yields the displayed closed form for \(\bar S_T^{\mathrm{ad}}\). The observable IPCW bound then follows from the identity
\[
\sum_{t=1}^T(\hat e_t-\alpha)=\frac{\bar S_T^{\mathrm{ad}}}{g_{\min}}-\alpha A_T.
\]
...
where
\[
A_T:=\sum_{t=1}^T \1\{\Delta_t=0,\ Y_t<L_t\},
\qquad
\lambda_T:=\sqrt{1+T},
\qquad
...
\paragraph{Proof sketch.}
Start from the decomposition of latent excess into observable IPCW excess minus the martingale noise term. Then apply Theorem~\ref{thm:adaftrl-surrogate-main} to the IPCW term, keeping the explicit \(-\alpha A_T\) correction produced by the ambiguity-neutral update, and finally apply the same one-sided Freedman step used in the shared statistical core to the martingale term. The full proof is recorded in Appendix~\ref{app:adaftrl-details}.
...
\]
In the AdaFTRL theorem application we evaluate this inverse at
\[
b=g_{\min}B_T^{\mathrm{ad}}\ge 0.
\]
...
\frac{\Delta_t}{G_t(Y_t)}\,\mathbf{1}\{Y_t< L_t\}.
\]
Now consider a censored round. Since $\Delta_t=0$, we automatically have $\hat e_t=0$, regardless of the value of $L_t$. This bundles together two logically different situations: the favorable case $Y_t\ge L_t$, where coverage is already certified, and the ambiguous case $Y_t<L_t$, where censoring occurred below the played lower bound and the latent event $\{T_t<L_t\}$ is still unknown. The instability comes from the second case, because the raw IPCW update still treats it as the value $0$. Therefore
\[
...
\tau_t=\tau_{\max}e^{-a_t}.
\]
In the final AdaFTRL construction we additionally neutralize the ambiguous censored-below-the-bound case by replacing its feedback value by $\alpha$, so that such rounds contribute zero internal gradient rather than artificial upward drift. That is why the paper is built around the exponential internal coordinate rather than a direct online calibration of $\tau_t$ itself.
...
\]
Finally substitute $h_t=-\bar g_t^{\mathrm{ad}}$ and multiply by $-1$ inside the sum:
\[
\sum_{t=1}^T \bar g_t^{\mathrm{ad}}(a-a_t)
\le
\frac{\Lambda_{T+1}}{2}(a-a_{\mathrm{init}})^2+\frac12\sum_{t=1}^T \frac{(\bar g_t^{\mathrm{ad}})^2}{\Lambda_t}.
...
In particular,
\[
\sum_{t=1}^T \frac{(\bar g_t^{\mathrm{ad}})^2}{\Lambda_t} \le (1+\sqrt{2})(\Lambda_{T+1}-1).
\]
\end{lemma}
...
\frac{1+\sqrt{2}}{2}(\Lambda_{T+1}-1).
\]
Since \(|\bar g_t^{\mathrm{ad}}|\le 1\), we also have
\[
...
\begin{lemma}[Corrected compatibility inequality for AdaFTRL]
\label{lem:compatibility-ad}
Under Assumptions~\ref{ass:monotone}, \ref{ass:gmin}, and \ref{ass:score-positive},
\[
a_t\bar g_t^{\mathrm{ad}}
\le
 g_{\min}\hat e_t^{\mathrm{ad}} b_t
\qquad\text{for every }t.
\]
\end{lemma}
...
\paragraph{Case 1: $Y_t\ge L_t$.}
Then $\hat e_t^{\mathrm{ad}}=0$ and
\[
\bar g_t^{\mathrm{ad}}=g_{\min}(0-\alpha)=-\alpha g_{\min}<0.
\]
Since $a_t\ge 0$, we have
\[
a_t\bar g_t^{\mathrm{ad}}\le 0 = g_{\min}\hat e_t^{\mathrm{ad}} b_t.
\]

\paragraph{Case 2: $\Delta_t=1$ and $Y_t<L_t$.}
Here $A_t=0$ and therefore $\hat e_t^{\mathrm{ad}}=\hat e_t=\widetilde W_t$. Hence
\[
\bar g_t^{\mathrm{ad}}=\bar g_t.
\]
Lemma~\ref{lem:compatibility} gives
\[
a_t\bar g_t^{\mathrm{ad}}=a_t\bar g_t\le g_{\min}\widetilde W_t b_t=g_{\min}\hat e_t^{\mathrm{ad}} b_t.
\]

\paragraph{Case 3: $\Delta_t=0$ and $Y_t<L_t$.}
Then $A_t=1$ and $\hat e_t=0$, so $\hat e_t^{\mathrm{ad}}=\alpha$ and therefore
\[
\bar g_t^{\mathrm{ad}}=g_{\min}(\alpha-\alpha)=0.
\]
Thus
\[
a_t\bar g_t^{\mathrm{ad}}=0\le g_{\min}\alpha b_t=g_{\min}\hat e_t^{\mathrm{ad}} b_t.
\]
This completes the proof.
\end{proof}

\begin{theorem}[Comparator-minus-algorithm Fenchel reduction for corrected AdaFTRL feedback]
...
\[
\sum_{t=1}^T \bar g_t^{\mathrm{ad}}(a-a_t)\le \mathcal R_T(a).
\]
Define
\[
\bar S_T^{\mathrm{ad}}:=\sum_{t=1}^T \bar g_t^{\mathrm{ad}},
...
Then
\[
\mathcal R_T^\star(\bar S_T^{\mathrm{ad}})\le g_{\min}B_T^{\mathrm{ad}}.
\]
Consequently,
\[
\bar S_T^{\mathrm{ad}}\le U_T(g_{\min}B_T^{\mathrm{ad}}),
\qquad
...
\[
a\sum_{t=1}^T \bar g_t^{\mathrm{ad}}-\sum_{t=1}^T a_t\bar g_t^{\mathrm{ad}}\le \mathcal R_T(a).
\]
By definition of $\bar S_T^{\mathrm{ad}}$, this is
\[
a\bar S_T^{\mathrm{ad}}-\mathcal R_T(a)\le \sum_{t=1}^T a_t\bar g_t^{\mathrm{ad}}.
\]
Now take the supremum over $a\ge 0$. By definition of the conjugate,
\[
\mathcal R_T^\star(\bar S_T^{\mathrm{ad}})\le \sum_{t=1}^T a_t\bar g_t^{\mathrm{ad}}.
\]
Because Assumptions~\ref{ass:monotone}, \ref{ass:gmin}, and \ref{ass:score-positive} hold, Lemma~\ref{lem:compatibility-ad} applies and yields
\[
a_t\bar g_t^{\mathrm{ad}}\le g_{\min}\hat e_t^{\mathrm{ad}} b_t
\qquad\text{for each }t.
\]
Summing over $t$ gives
\[
\sum_{t=1}^T a_t\bar g_t^{\mathrm{ad}}\le g_{\min}\sum_{t=1}^T \hat e_t^{\mathrm{ad}} b_t=g_{\min}B_T^{\mathrm{ad}}.
\]
...
U_T(b):=\sup\{z\in\R:\ \mathcal R_T^\star(z)\le b\},
\]
the inequality $\mathcal R_T^\star(\bar S_T^{\mathrm{ad}})\le g_{\min}B_T^{\mathrm{ad}}$ implies
\[
\bar S_T^{\mathrm{ad}}\le U_T(g_{\min}B_T^{\mathrm{ad}}).
\]
\end{proof}
...
We proceed in three steps: compute the conjugate explicitly, apply the corrected black-box theorem, and then solve the resulting quadratic inequality.
...
\paragraph{Step 2: plug the conjugate into the corrected black-box theorem.}
...
\mathcal R_T^\star(\bar S_T^{\mathrm{ad}})\le g_{\min}B_T^{\mathrm{ad}}.
\]
If $\bar S_T^{\mathrm{ad}}\le 0$, then the final theorem is automatic because the claimed upper bound on $\bar S_T^{\mathrm{ad}}$ is nonnegative. So only the case $\bar S_T^{\mathrm{ad}}>0$ needs work.
Because $a_{\mathrm{init}}\ge 0$ and $\lambda_T>0$, the threshold $-\lambda_T a_{\mathrm{init}}$ is nonpositive. Therefore any positive value of $\bar S_T^{\mathrm{ad}}$ lies in the first branch of the conjugate. We may thus write
\[
a_{\mathrm{init}}\bar S_T^{\mathrm{ad}}+\frac{(\bar S_T^{\mathrm{ad}})^2}{2\lambda_T}-\frac{1+\sqrt{2}}{2}(\lambda_T-1)\le g_{\min}B_T^{\mathrm{ad}}.
\]

\paragraph{Step 3: solve the quadratic inequality carefully and return to the IPCW quantity.}
Move every term to the left-hand side and multiply by $2\lambda_T$:
\[
(\bar S_T^{\mathrm{ad}})^2+2a_{\mathrm{init}}\lambda_T\bar S_T^{\mathrm{ad}}
-
2\lambda_T\Bigl(g_{\min}B_T^{\mathrm{ad}}+\frac{1+\sqrt{2}}{2}(\lambda_T-1)\Bigr)\le 0.
\]
...
\qquad
c=2\lambda_T\Bigl(g_{\min}B_T^{\mathrm{ad}}+\frac{1+\sqrt{2}}{2}(\lambda_T-1)\Bigr).
\]
Since \(\bar S_T^{\mathrm{ad}}\ge 0\), the relevant bound is the nonnegative root, which simplifies to
\[
\bar S_T^{\mathrm{ad}}
\le
-a_{\mathrm{init}}\lambda_T
...
Finally use
\[
\bar S_T^{\mathrm{ad}}=g_{\min}\sum_{t=1}^T(\hat e_t^{\mathrm{ad}}-\alpha)
=g_{\min}\sum_{t=1}^T(\hat e_t-\alpha)+\alpha g_{\min}A_T,
\]
which is equivalent to
\[
\sum_{t=1}^T(\hat e_t-\alpha)=\frac{\bar S_T^{\mathrm{ad}}}{g_{\min}}-\alpha A_T.
\]
Substituting the bound on \(\bar S_T^{\mathrm{ad}}\) proves the theorem.
\end{proof}
...
\]
Apply Theorem~\ref{thm:adaftrl-surrogate-main} to the IPCW term to obtain
\[
\sum_{t=1}^T (\hat e_t-\alpha)
\le
\frac{1}{g_{\min}}
